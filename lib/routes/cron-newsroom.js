...
function currentWindow(clock) {
  // CATCH-UP RULE:
  // Select the most recent publishing window that should already exist today.
  // If midnight/afternoon/evening/late-breaking was missed because of a deploy,
  // delayed cron, or temporary outage, the next cron wake-up creates it.
  const eligible = EDITION_WINDOWS
    .filter(item => item.hour <= clock.hour)
    .sort((a, b) => b.hour - a.hour);

  return eligible[0] || EDITION_WINDOWS[0];
}
  ...
  try {
    ...
    if (!run) {
      return json(res, 200, {
        ...
        scheduled,
        activeRun: false,
        expectedEditionSlot: currentWindow(clock)?.slot || null,
        expectedEditionLabel: currentWindow(clock)?.label || null,
        message: scheduled
