...
import { repairCheckpointLeads } from '../checkpoint-lead-repair.js';
...
function sourceSelectionScore(story={}){
  ...
  return (Number.isFinite(reach)?reach:0)*10000+(String(story.sourceSelectionNote||'').trim()?500:0)+longevity+score(story);
}
function best(rows,eraKey,predicate=()=>true){
  return [...rows].filter(s=>s?.eraKey===eraKey&&s?.title&&s?.sourceUrl&&predicate(s)).sort((a,b)=>sourceSelectionScore(b)-sourceSelectionScore(a))[0]||null;
}
...
function assembleFromVerified(run,verified,existingPayload={}){
  ...
  const leadTopicKey=y100Major?.topicKey||'';
  ...
  for(const story of rows.filter(s=>s?.eraKey==='y100'&&s?.title&&s?.sourceUrl)){
    ...
    const relation=s=>leadTopicKey&&s.comparisonType==='same_topic'&&s.topicKey===leadTopicKey?2:leadEventKey&&s.eventKey===leadEventKey?3:s.comparisonType==='community_lead'&&s.searchOutcome==='same_topic_not_verified'&&String(s.sourceSelectionNote||'').trim()?1:0;
    if(!relation(story))continue;
    if(!prior || relation(story)>relation(prior) || (relation(story)===relation(prior)&&sourceSelectionScore(story)>sourceSelectionScore(prior))) byCommunity.set(key,story);
  }
  const communityTiles=[...byCommunity.values()].slice(0,12).map(s=>({
    ...s,
    comparisonType:s.comparisonType==='same_topic'?'same_topic':leadEventKey&&s.eventKey===leadEventKey?'same_event':s.comparisonType,
    searchOutcome:s.comparisonType==='same_topic'?'same_topic_verified':leadEventKey&&s.eventKey===leadEventKey?'same_event_verified':s.searchOutcome,
  }));
...
async function latestDailyRun(){
  const runs=await select('otd_runs','select=*&run_kind=like.daily*&order=created_at.desc&limit=1').catch(()=>[]);
  return runs[0]||null;
}
...
async function repairEditionFromVerified(){
  ...
  const existingExact=edition?sanitizeExactDateEdition(edition.payload||{},run.edition_date):null;
  if(edition&&edition.status==='published'&&existingExact?.complete){
  ...
  const assembled=assembleFromVerified(run,verified,edition?.payload||{});
  if(!assembled) return {repaired:false,reason:'verified_output_empty',runId:run.id};
  const payload=applyEditorialSupplements(assembled,run.edition_date);
  const publication=await publishEditionSlots(run,{
    slots:{
      y200:payload.stories?.y200,
      y100:payload.stories?.y100?.major,
      y75:payload.stories?.y75,
    },
    basePayload:payload,
    policy:'single_verified_cron_repair_v1',
  });
  if(!publication?.published||!publication?.complete){
    return {repaired:false,reason:'verified_output_incomplete',runId:run.id,quality:editionQuality(payload),publication};
  }

  const now=new Date().toISOString();
  await update('otd_runs',`id=eq.${run.id}`,{error:null,updated_at:now}).catch(()=>{});
  return {repaired:true,runId:run.id,editionId:publication.editionId,quality:editionQuality(publication.payload)};
}
...
async function recoverEmptyPublishedEdition(){
  ...
  const quality=sanitizeExactDateEdition(edition.payload||{},run.edition_date);
...
export default async function handler(req,res){
  ...
  try {
    ...
    // Once every canonical agent has completed, preserve those outputs. If the
    // checkpoint publication is missing exact-date/circulation-first national
    // leads, run ONLY the small lead-repair subdesk; never restart all 19 agents.
    if(checkpoint.locked){
      const leadRepair=await repairCheckpointLeads(run,checkpoint);
      return json(res,200,{ok:true,automatic:true,trigger:'vercel_cron',checkpoint,checkpointLocked:true,leadRepair});
    }
    ...
    if(postCheckpoint.locked){
      const leadRepair=await repairCheckpointLeads(afterRun,postCheckpoint);
      return json(res,result.ok===false?500:200,{...result,automatic:true,trigger:'vercel_cron',checkpoint:postCheckpoint,checkpointLocked:true,leadRepair,publicationRecovery,verificationRecovery});
    }
