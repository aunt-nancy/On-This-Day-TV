...
async function recoverEmptyPublishedEdition(){
  ...
  // Any approvals attached to an edition that failed the publication gate are
  // decisions about discarded content. Do not let them survive into the next
  // verification/research pass and block a newly rebuilt edition.
...
export default async function handler(req,res){
  ...
  try {
    ...
    // If this very tick wrote an incomplete edition, reject it immediately
    // rather than leaving a false "published" row visible until the next cron.
