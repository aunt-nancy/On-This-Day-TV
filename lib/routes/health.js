...
import { rosterSummary, siteDate, STAGES } from '../engine.js';
...
export default async function handler(req,res){
  ...
  const [runs,editions,published]=await Promise.all([
    select('otd_runs',`select=id,edition_date,status,stage,run_attempt,next_retry_at,error,created_at,updated_at&edition_date=eq.${editionDate}&run_kind=eq.daily&limit=1`).catch(()=>[]),
