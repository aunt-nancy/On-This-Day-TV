  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      const retryPrompt = attempt === 1 ? prompt : {
        ...prompt,
        instructions: `${prompt.instructions}\nRETRY RULE: The prior attempt failed. Return a smaller response, valid JSON only, with no markdown and no commentary. Preserve only the strongest supported candidates.`,
      };
...
async function executeNonfatalAgent(runId, agentKey, fn) {
  try {
    return await executeAgent(runId, agentKey, fn);
  } catch (error) {
    return {
      agent: agentKey,
      status: 'failed',
...
export async function runAllAgents({ date, trigger = 'manual' } = {}) {
  ...
  try {
    ...
    const researchOutputs = await Promise.all(researchKeys.map(agentKey =>
      executeNonfatalAgent(run.id, agentKey, () =>
        modelJson(
    ...
    const researchFailures = researchOutputs
      .filter(output => output?.status === 'failed')
      .map(output => ({
        type: 'agent_failure',
        agent: output.agent,
        description: output.error || output.discrepancies?.[0]?.description || 'Research agent failed',
      }));
    ...
    const discrepancy = await executeAgent(run.id, 'discrepancy_exception', () =>
      modelJson(discrepancyPrompt(context, { verified, contextual, translation, rights, visuals, researchFailures }), { model: verifyModel, webSearch: false, reasoning: 'high' })
    );
