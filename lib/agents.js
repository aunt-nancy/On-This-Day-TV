import { siteDateParts, siteDateISO, parseExplicitDate, monthName, SITE_TIME_ZONE } from './site-date.js';
export const EXPECTED_AGENT_COUNT = 19;
export const AGENT_ROSTER_VERSION = '2026-08-30.roster19.1';
...
export function computeEditionDate(inputDate) {
  ...
  if (inputDate) {
    const explicit = parseExplicitDate(inputDate);
    iso = explicit.iso;
    year = explicit.year;
    monthNumber = explicit.month;
    day = explicit.day;
  } else {
    const parts = siteDateParts();
    iso = siteDateISO();
    year = parts.year;
  ...
  return {
    ...
    // Internal key y76 is retained for schema compatibility.
    // Visitor-facing approved historical window is 75 years ago.
    years: {
      y200: year - 200,
      y100: year - 100,
      y76: year - 75,
...
export function sourceRules() {
  return {
    ...
    searchRules: [
      ...
      'For y76/1951-era research, search national papers of record AND multiple major credible regional newspapers before concluding that no strong headline exists.',
