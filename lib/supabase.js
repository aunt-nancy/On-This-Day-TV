...
function guardEditionWrites(table, rows){
  if(table!=='otd_editions') return rows;
  const list=Array.isArray(rows)?rows:[rows];
  const guarded=list.map(row=>{
    if(!row||typeof row!=='object'||row.status!=='published') return row;
    const policy=String(row?.payload?.publisherPolicy||'');
    if(policy.startsWith('single_')) return row;
    // Legacy newsroom code may assemble/stage an edition, but publication is
    // owned exclusively by lib/publisher.js. This prevents competing writers
    // from restoring off-date or stale stories after the authoritative writer.
    return {...row,status:'preparing',published_at:null};
  });
  return Array.isArray(rows)?guarded:guarded[0];
}
...
export async function upsert(table, rows, onConflict, {returning=true}={}) {
  ...
  const guarded=guardEditionWrites(table,rows);
  return request(`${rest(table)}${q}`, { method:'POST', body:JSON.stringify(Array.isArray(guarded)?guarded:[guarded]), prefer:`resolution=merge-duplicates,${returning?'return=representation':'return=minimal'}` });
}
