...
export async function createQueuedRun({ date, trigger = 'manual' } = {}) {
  ...
  for (const old of stale) {
    await update('agent_runs', `id=eq.${old.id}`, {
      status: 'failed',
      error: 'Superseded by a new prioritized staged run.',
      finished_at: now(),
    }).catch(() => {});
  ...
  const [run] = await insert('agent_runs', {
    ...
    status: 'running',
    trigger,
    agent_version: '2026-08-30.priority1-video-stable16.1',
    years: context.years,
    started_at: now(),
  });
  return { run, context };
  ...
  try {
    ...
    if (agentKey === 'editor_opening') {
      output = await modelJson(
        ...
        { model: verifyModel, webSearch: false, reasoning: 'medium' }
      );
    } else if (RESEARCH_KEYS.includes(agentKey)) {
      const opening = await completedOutput(runId, 'editor_opening') || {};
      output = await modelJson(
        researchPrompt(agentKey, context, opening.agenda || {}),
        { model, webSearch: true, reasoning: 'medium' }
      );
    } else {
      const opening = await completedOutput(runId, 'editor_opening') || {};
      ...
      if (agentKey === 'historical_context') {
        ...
        if (!translatable.length) {
          ...
          output = await modelJson(
            ...
            { model, webSearch: false, reasoning: 'low' }
          );
        }
        ...
        output = await modelJson(
          ...
          { model, webSearch: true, reasoning: 'low' }
        );
        ...
        try {
          output = await modelJson(
            editorPrompt(context, { ...verified, verifiedStories: safeStories }, contextual, rights, chooseVisuals(visuals), discrepancy, opening.agenda || {}),
            { model: verifyModel, webSearch: false, reasoning: 'high' }
          );
        } catch (editorError) {
        ...
        output = await modelJson(
          socialPrompt('social_editor', context, editor.edition),
          { model, reasoning: 'low' }
        );
        ...
        try {
          output = await modelJson(
            socialPrompt('short_form_video', context, editor.edition),
            {
              model,
              reasoning: 'low',
          ...
          // Priority-1 stability rule: Short-Form Video is optional and can
          // never turn an otherwise successful 16-agent newsroom run into a failure.
          output = {
            ...
            confidence: 0,
            skipped: true,
            reason: `Short-form video response was unusable and was safely skipped: ${videoError.message}`,
            discrepancies: [],
          };
        }
