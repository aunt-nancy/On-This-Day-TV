...
export async function createQueuedRun({ date, trigger = 'manual' } = {}) {
  ...
  const [run] = await insert('agent_runs', {
    ...
    status: 'running',
    trigger,
    agent_version: '2026-08-30.parallel.1',
    years: context.years,
    started_at: now(),
  });
  return { run, context };
    ...
    if (agentKey === 'editor_opening') {
      ...
      if (agentKey === 'historical_context') {
        ...
        output = await modelJson(
          ...
          { model: verifyModel, webSearch: true, reasoning: 'medium', maxOutputTokens: 6000 }
        );
        ...
        const immediate = await persistEditionCore(run, context, immediateOutput, verified, discrepancy);
        ...
        try {
          ...
          const polishedPersist = await persistEditionCore(run, context, polished, verified, discrepancy);
          output = {
            ...polished,
