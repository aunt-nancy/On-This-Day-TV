...
export async function createQueuedRun({ date, trigger = 'manual' } = {}) {
  ...
  const [run] = await insert('agent_runs', {
    ...
    status: 'running',
    trigger,
    agent_version: '2026-08-30.19agent-agent13-15-direct.1',
    years: context.years,
    started_at: now(),
  });
  ...
  try {
    ...
    if (agentKey === 'editor_opening') {
      ...
      if (agentKey === 'historical_context') {
        ...
        // Storage is a prerequisite for usable artwork. Verify/create it once
        // before spending image-generation time.
