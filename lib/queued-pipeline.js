...
import {
  ...
  sideEraRecoveryPrompt, visualArchivePrompt, rightsPrompt, discrepancyPrompt, editorPrompt, socialPrompt, trendsPrompt,
} from './prompts.js';
...
export async function createQueuedRun({ date, trigger = 'manual' } = {}) {
  ...
  const [run] = await insert('agent_runs', {
    ...
    status: 'running',
    trigger,
    agent_version: '2026-08-30.illustrator-hard-activation.1',
    years: context.years,
    started_at: now(),
  });
  return { run, context };
  ...
  try {
    ...
    if (agentKey === 'editor_opening') {
      ...
      if (agentKey === 'historical_context') {
        ...
        const editor = await completedOutput(runId, 'editor_producer');
        const edition = editor?.edition || editor;
        if (!edition) throw new Error('Published edition is unavailable for short-form video.');

        try {
          output = await modelJson(
            socialPrompt('short_form_video', context, edition),
            { model, webSearch: false, reasoning: 'low', maxOutputTokens: 1600 }
          );
        } catch (videoError) {
          // Social video is optional and must never make a successful newsroom
          // run look failed because the model returned malformed/truncated JSON.
          output = {
