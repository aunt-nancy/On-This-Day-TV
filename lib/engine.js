...
import {
  editorOpeningPrompt, researchPrompt, contextPrompt, translationPrompt,
...
async function automaticRetryIfDue(run) {
  ...
  const due=!run.next_retry_at || new Date(run.next_retry_at).getTime()<=Date.now();
  if(!due) return null;
...
export async function advanceRun(run) {
  ...
  try {
    ...
    switch(current.stage) {
      ...
      case 'major_research': {
        await runAgent(current,'major_press',context,await depsFor(current,'major_press',context));
    ...
    const waitMinutes=Math.min(360,15*Math.pow(2,Math.min(attempt-1,5)));
