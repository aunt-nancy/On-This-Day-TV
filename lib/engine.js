...
import {
  ...
  verificationPrompt, verificationBatchPrompt, visualArchivePrompt, rightsPrompt, discrepancyPrompt,
...
const VERIFY_SUB_MAX_ATTEMPTS = 4;
...
export const UPDATE_WINDOWS = Object.freeze([
  {key:'0000',hour:0,minute:0,label:'12:00 AM'},
  {key:'0600',hour:6,minute:0,label:'6:00 AM'},
  {key:'0900',hour:9,minute:0,label:'9:00 AM'},
  {key:'1200',hour:12,minute:0,label:'12:00 PM'},
  {key:'1500',hour:15,minute:0,label:'3:00 PM'},
  {key:'1800',hour:18,minute:0,label:'6:00 PM'},
  {key:'2200',hour:22,minute:0,label:'10:00 PM'},
].map(window=>Object.freeze({...window,runKind:`daily_${window.key}`})));
...
export function currentUpdateWindow(now = new Date(), timeZone = SITE_TIME_ZONE) {
  const parts=new Intl.DateTimeFormat('en-US',{
    timeZone,hour:'2-digit',minute:'2-digit',hourCycle:'h23',
  }).formatToParts(now);
  const value=type=>Number(parts.find(part=>part.type===type)?.value||0);
  const minuteOfDay=value('hour')*60+value('minute');
  return [...UPDATE_WINDOWS].reverse().find(window=>window.hour*60+window.minute<=minuteOfDay)||UPDATE_WINDOWS[0];
}
...
function verificationCandidateId(candidate={}) {
  return String(candidate.sourceUrl || `${candidate.eraKey||''}|${candidate.sourceDesk||''}|${candidate.publication||''}|${candidate.title||''}`);
}

export function buildVerificationBatches(research=[]) {
  const candidates=safeArray(research).filter(c=>c && c.eraKey && (c.sourceUrl || c.title));
  const batches=[];
  for(const eraKey of ['y100','y200','y75']) {
    const era=candidates.filter(c=>c.eraKey===eraKey);
    if(!era.length) continue;
    if(eraKey==='y100') {
      const anchors=era.filter(c=>c.sourceDesk==='major_press').slice(0,1);
      const anchorIds=new Set(anchors.map(verificationCandidateId));
      const others=era.filter(c=>!anchorIds.has(verificationCandidateId(c)));
      if(!others.length) batches.push({eraKey,candidates:anchors,anchors});
      else {
        for(let i=0;i<others.length;i+=Math.max(1,VERIFY_BATCH_SIZE-anchors.length)) {
          const chunk=others.slice(i,i+Math.max(1,VERIFY_BATCH_SIZE-anchors.length));
          batches.push({eraKey,candidates:[...anchors,...chunk],anchors});
        }
      }
    } else {
      for(let i=0;i<era.length;i+=VERIFY_BATCH_SIZE) batches.push({eraKey,candidates:era.slice(i,i+VERIFY_BATCH_SIZE),anchors:[]});
    }
  }
  return batches.map((batch,index)=>{
    const signature=batch.candidates.map(verificationCandidateId).sort().join('|');
    const hash=crypto.createHash('sha1').update(signature).digest('hex').slice(0,10);
    return {...batch,index,agentKey:`source_verification_${batch.eraKey}_${hash}`};
  });
}

async function runVerificationBatch(run,batch,context,contextual={}) {
  let previous=await failStale(await latestJob(run.id,batch.agentKey));
  if(previous?.status==='complete') return {state:'complete',output:previous.output};
  if(previous?.status==='running') return {state:'running',output:null};
  const attempt=Math.max(0,Number(previous?.attempt||0))+1;
  if(attempt>VERIFY_SUB_MAX_ATTEMPTS) return {state:'terminal_failed',output:null,error:previous?.error||'Maximum verification-batch attempts exceeded'};

  const [job]=await insert('otd_agent_jobs',{run_id:run.id,agent_key:batch.agentKey,status:'running',attempt,started_at:nowIso()});
  try {
    const p=verificationBatchPrompt(context,batch.candidates,batch.anchors,contextual);
    const result=await runModel({
      instructions:p.instructions,
      input:p.input,
      model:modelFor('source_verification'),
      webSearch:true,
      reasoning:'low',
    ...
    const output=result.json||{};
    output.verifiedStories=safeArray(output.verifiedStories);
    output.rejectedCandidates=safeArray(output.rejectedCandidates);
    output.discrepancies=safeArray(output.discrepancies);
    await update('otd_agent_jobs',`id=eq.${job.id}`,{status:'complete',output,confidence:num(output?.confidence),error:null,finished_at:nowIso()});
    return {state:'complete',output};
  } catch(error) {
    await update('otd_agent_jobs',`id=eq.${job.id}`,{status:'failed',error:error.message,finished_at:nowIso()}).catch(()=>{});
    return {state:attempt>=VERIFY_SUB_MAX_ATTEMPTS?'terminal_failed':'failed',output:null,error:error.message};
  }
}
...
function importanceScore(value) {
  ...
  if(/very high|national|major|critical|highest/.test(text)) return 5;
  if(/high|significant/.test(text)) return 4;
  if(/medium|moderate/.test(text)) return 3;
  if(/low|local/.test(text)) return 2;
  return 1;
}

function chooseLead(stories,eraKey) {
  const era=safeArray(stories).filter(s=>s.eraKey===eraKey);
  if(!era.length) return '';
  const major=era.filter(s=>s.sourceDesk==='major_press');
  const pool=major.length?major:era;
  if(eraKey==='y100') {
    const supportCounts=new Map();
    for(const story of era) if(story.eventKey && story.sourceDesk!=='major_press') supportCounts.set(story.eventKey,(supportCounts.get(story.eventKey)||0)+1);
    return [...pool].sort((a,b)=>{
      const sa=(supportCounts.get(a.eventKey)||0)*100+importanceScore(a.nationalImportance)*10+num(a.confidence);
      const sb=(supportCounts.get(b.eventKey)||0)*100+importanceScore(b.nationalImportance)*10+num(b.confidence);
      return sb-sa;
    })[0]?.eventKey||'';
  }
  return [...pool].sort((a,b)=>(importanceScore(b.nationalImportance)*10+num(b.confidence))-(importanceScore(a.nationalImportance)*10+num(a.confidence)))[0]?.eventKey||'';
}

async function ensureVerificationCoordinator(run) {
  const existing=await latestJob(run.id,'source_verification');
  if(existing?.status==='complete') return existing;
  if(existing?.status==='running') return existing;
  const attempt=Math.max(0,Number(existing?.attempt||0))+1;
  const [job]=await insert('otd_agent_jobs',{run_id:run.id,agent_key:'source_verification',status:'running',attempt,started_at:nowIso()});
  return job;
}

async function finalizeSourceVerification(run,batches) {
  const existing=await completeOutput(run.id,'source_verification');
  if(existing) return existing;
  const outputs=[];
  for(const batch of batches) {
    const out=await completeOutput(run.id,batch.agentKey);
    if(!out) return null;
    outputs.push(out);
  }
  const byIdentity=new Map();
  for(const story of outputs.flatMap(o=>safeArray(o.verifiedStories))) {
    const id=keyOfStory(story); const prior=byIdentity.get(id);
    if(!prior || num(story.confidence)>=num(prior.confidence)) byIdentity.set(id,story);
  }
  const verifiedStories=[...byIdentity.values()];
  const recommendedLeadByEra={
    y200:chooseLead(verifiedStories,'y200'),
    y100:chooseLead(verifiedStories,'y100'),
    y75:chooseLead(verifiedStories,'y75'),
  };
  const confidence=outputs.length?outputs.reduce((sum,o)=>sum+num(o.confidence),0)/outputs.length:0;
  const merged={
    agent:'source_verification',status:'complete',strategy:'bounded_candidate_batches_v1',confidence,
    recommendedLeadEventKey:recommendedLeadByEra.y100,
    recommendedLeadByEra,
    verifiedStories,
    rejectedCandidates:outputs.flatMap(o=>safeArray(o.rejectedCandidates)),
    discrepancies:outputs.flatMap(o=>safeArray(o.discrepancies)),
    batchCount:batches.length,
  };
  const coordinator=await latestJob(run.id,'source_verification');
  if(coordinator?.status==='running') await update('otd_agent_jobs',`id=eq.${coordinator.id}`,{status:'complete',output:merged,confidence,error:null,finished_at:nowIso()});
  else {
    const attempt=Math.max(0,Number(coordinator?.attempt||0))+1;
    const [job]=await insert('otd_agent_jobs',{run_id:run.id,agent_key:'source_verification',status:'running',attempt,started_at:nowIso()});
    await update('otd_agent_jobs',`id=eq.${job.id}`,{status:'complete',output:merged,confidence,error:null,finished_at:nowIso()});
  }
  return merged;
}

async function runSourceVerification(run,context,deps={}) {
  const existing=await completeOutput(run.id,'source_verification');
  if(existing) return {state:'complete',output:existing};
  const batches=buildVerificationBatches(deps.research||[]);
  if(!batches.length) throw new Error('source_verification has no research candidates to verify');
  await ensureVerificationCoordinator(run);

  const pending=[];
  for(const batch of batches) {
    const job=await latestJob(run.id,batch.agentKey);
    if(job?.status!=='complete') pending.push(batch);
  }
  const chosen=pending.slice(0,2);
  if(chosen.length) {
    const results=await Promise.all(chosen.map(batch=>runVerificationBatch(run,batch,context,deps.contextual||{})));
    const terminal=results.find(r=>r.state==='terminal_failed');
    if(terminal) throw new Error(`source_verification batch failed after ${VERIFY_SUB_MAX_ATTEMPTS} attempts: ${terminal.error}`);
  }
  const merged=await finalizeSourceVerification(run,batches);
  return merged?{state:'complete',output:merged}:{state:'running',output:null};
}
...
function sourceSelectionScore(story={}){
  const reach=num(story.historicalReachScore);
  ...
  return reach*10000+(String(story.sourceSelectionNote||'').trim()?500:0)+longevity+num(story.nationalImportance)*100+num(story.confidence);
}
function normalizeCommunityRelationship(story,lead={}){
  if(!story)return null;
  const leadEventKey=String(lead.eventKey||'');
  const leadTopicKey=String(lead.topicKey||'');
  const eventKey=String(story.eventKey||'');
  const topicKey=String(story.topicKey||'');
  if(story.comparisonType==='same_topic'&&leadTopicKey&&topicKey===leadTopicKey){
    return {...story,searchOutcome:'same_topic_verified'};
  }
  if(leadEventKey&&eventKey===leadEventKey){
    return {...story,comparisonType:'same_event',searchOutcome:'same_event_verified'};
  }
  if(
    story.comparisonType==='community_lead'&&
    story.searchOutcome==='same_topic_not_verified'&&
    String(story.sourceSelectionNote||'').trim()
  )return story;
  return null;
}
export function validateEdition(raw={},verified={}) {
  ...
  edition.leadTopicKey=lead?.topicKey||edition.leadTopicKey||'';
  ...
  edition.communityTiles=safeArray(edition.communityTiles)
    .map(cleanStory).map(s=>normalizeCommunityRelationship(s,lead)).filter(Boolean)
    .sort((a,b)=>{
...
async function publish(run,context) {
  ...
  let edition=validateEdition(result.output?.edition||{},safeVerified);
  ...
  edition=applyEditorialSupplements(edition,run.edition_date);
  const publication=await publishEditionSlots(run,{
    slots:{
      y200:edition.stories?.y200,
      y100:edition.stories?.y100?.major,
      y75:edition.stories?.y75,
    },
    basePayload:edition,
    policy:'single_newsroom_publish_v1',
  });
  if(!publication?.published||!publication?.complete)return null;
  await update('otd_approvals',`run_id=eq.${run.id}&edition_id=is.null`,{edition_id:publication.editionId}).catch(()=>{});
  return publication;
}
...
export async function ensureDailyRun(date=siteDate(),runKind=currentUpdateWindow().runKind) {
  const rows=await select('otd_runs',`select=*&edition_date=eq.${date}&run_kind=eq.${runKind}&order=created_at.desc&limit=1`).catch(()=>[]);
  if(rows[0]) return rows[0];
  const context=computeEditionDate(date);
  const inserted=await insert('otd_runs',{edition_date:date,run_kind:runKind,status:'queued',stage:'opening',run_attempt:1,years:context.years,created_at:nowIso(),updated_at:nowIso()});
  return inserted[0];
}
...
export async function advanceRun(run) {
  ...
  try {
    ...
    switch(current.stage) {
      ...
      case 'verification': {
        await runSourceVerification(current,context,await depsFor(current,'source_verification',context));
...
export async function tick(now=new Date()) {
  ...
  const run=await ensureDailyRun(siteDate(now),currentUpdateWindow(now).runKind);
