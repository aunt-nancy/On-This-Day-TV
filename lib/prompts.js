...
Standing discovery categories include Latino/Spanish-language, German American, British American, Irish American, Chinese American, Italian American, Jewish American, Japanese American, Indigenous/Native, Caribbean, South Asian, Filipino, Armenian, Greek, Polish and others. Use this priority list: ${COMMUNITY_PRIORITY.join(', ')}. Set sourceDesk="community_press". Never invent a candidate to fill a category.`,
  };
...
export function sideEraRecoveryPrompt(context, eraKey) {
  const eraYear = context?.years?.[eraKey];
  const label = eraKey === 'y200' ? '200 years ago' : '75 years ago';
  return {
    instructions: `${JSON_ONLY}
You are the Source Verification Agent performing a focused recovery search because the ${label} lead tile is missing.

Find and independently verify ONE of the most important American newspaper headlines/events from the exact historical date in ${eraYear}.
Use web search and institutional newspaper/archive sources.

PRIORITY:
- a genuine major national or broadly consequential American headline when one exists;
- federal/state government, elections, courts, war/diplomacy, major economic/labor developments, major disasters/crises, transportation/technology milestones, or other events that clearly led important news that day;
- for 1826-era coverage, recognize that the press was more fragmented: if no demonstrably national headline exists, choose the most consequential well-supported American headline available rather than a trivial local society item.

DO NOT use a routine marriage, society notice, minor municipal item, ordinary local business notice, or other filler when stronger verified coverage exists.
Do not invent a newspaper, issue, headline, date, page, archive, or URL.

Return exactly:
{"agent":"side_era_recovery","status":"complete","eraKey":"${eraKey}","verifiedStory":{"eraKey":"${eraKey}","eraYear":${eraYear},"eventKey":"","sourceDesk":"major_press","coverageScope":"national|multi_state|regional|local","nationalImportance":0,"title":"","summary":"","publication":"","city":"","issueDate":"","page":"","archive":"","sourceUrl":"","community":"major_press","language":"English","articleType":"news|editorial|other","confidence":0,"verificationNotes":""},"discrepancies":[]}.
If no reliable story can be verified, set verifiedStory to null and explain in discrepancies.`,
    input: JSON.stringify({
      editionDate: context.editionDate,
      month: context.month,
      day: context.day,
...
Determine whether the verified historical lead has a REAL, meaningful, sourced connection to the present day.

STRICT RULE:
If the connection is weak, superficial, merely thematic, or cannot be supported by reliable present-day sources, return show=false.

If meaningful, return:
{"agent":"then_now","status":"complete","show":true,"then":{"title":"","text":""},"changed":{"title":"","text":""},"now":{"title":"","text":""},"sources":[{"label":"","url":""}],"confidence":0,"discrepancies":[]}

Requirements:
- THEN: historical situation from the verified edition.
- WHAT CHANGED: concrete legal, social, technological, political, economic, or institutional changes.
- NOW: present-day status.
- Include at least one historical source and one reliable present-day source.
- Do not force a connection.
- Keep each text block under 80 words.`,
