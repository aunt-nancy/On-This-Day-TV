...
export function researchPrompt(agentKey, context) {
  ...
  const roles = {
    ...
    community_press: `Find relevant community newspapers, including Latino/Spanish-language, German American, British American, Irish American, Chinese American, Italian American, Jewish American, Japanese American, Indigenous/Native, Caribbean, South Asian, Filipino, Armenian, Greek, Polish and others. Use this standing priority list: ${COMMUNITY_PRIORITY.join(', ')}. Do NOT attempt exhaustive coverage of every community in one run. Return at most 10 total candidates, prioritizing communities with the strongest source-supported connection to the day's headlines and larger population significance. British American and German American remain standing discovery categories, but return a candidate only when a real source supports it. Keep searchNotes concise.`,
...
export function discrepancyPrompt(context, components) {
  return {
    instructions: `${JSON_ONLY}\nYou are the Discrepancy & Exception Agent. Human intervention is required ONLY for unresolved discrepancies. Classify all flags. Return {"agent":"discrepancy_exception","status":"complete","publishable":true,"blocking":[],"nonBlocking":[],"humanReviewRequired":false,"confidence":0}. Each blocking item MUST include {"scope":"story|edition","eventKey":"","sourceUrl":"","description":"","evidence":{}} when those identifiers are known. Use scope="edition" only when the entire edition is genuinely unsafe (for example, the date/year calculation itself is unresolved). Story-level discrepancies must NOT block unrelated verified stories. Blocking examples: conflicting dates, doubtful attribution, unsupported quotation/headline, uncertain translation used as a quotation, unresolved rights conflict, or material factual disagreement. Ordinary low-risk workflow does not require human approval.`,
...
export function editorPrompt(context, verified, contextual, rights, visuals, discrepancy) {
  return {
    instructions: `${JSON_ONLY}\nYou are the Editor & Producer Agent. Build the final On This Day edition. The 100-year desk is dominant. African American / Black Press is always the center comparison. Do not put illustrations inside the center comparison tile. Keep the locked horizontal masthead unchanged. Use minimal timely illustrations elsewhere. Return {"agent":"editor_producer","status":"complete","edition":{"editionDate":"","leadHeadline":"","years":{},"stories":{"y200":{},"y100":{"major":{},"black":{},"secondary":[]},"y76":{}},"communityTiles":[],"visuals":[],"sourceSummary":[],"publishedStoryKeys":[],"heldForReview":[],"publicationStatus":"draft|published|needs_human"},"confidence":0}. Every story object should preserve eventKey, sourceUrl, publication, title, summary, and eraKey when available. Omit any story explicitly identified by a story-level blocking discrepancy and list it in heldForReview instead. A story-level discrepancy must NOT hold the whole edition. Set publicationStatus="published" whenever at least one verified, undisputed story remains and there is no edition-level blocking discrepancy. Use needs_human only when an edition-level discrepancy blocks the entire edition or no safe story remains.`,
