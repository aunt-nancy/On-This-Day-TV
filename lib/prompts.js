...
The Major American Press desk establishes the lead-event candidates first. Black Press, Regional/Local, and Community Press desks then search specifically for coverage or reaction to those same candidates, followed by clearly labeled representative community leads when no response survives.
...
Each candidate should use {"eraKey":"y200|y100|y75","eraYear":0,"eventKey":"stable_same_event_slug","sourceDesk":"major_press|black_press|regional_local|community_press","comparisonType":"same_event|community_lead|unmatched","dateRelation":"exact_date|nearest_weekly_issue","coverageScope":"national|multi_state|regional|local","nationalImportance":0,"title":"","summary":"","publication":"","city":"","issueDate":"YYYY-MM-DD or null","page":"","archive":"","sourceUrl":"","community":"","language":"","articleType":"news|editorial|letter|advertisement|reprint|other","evidenceNotes":"","confidence":0}.`;
  ...
  const roles = {
    ...
    black_press: 'COMMUNITY LENS DESK — BLACK PRESS. For y100, FIRST search specifically for African American / Black Press reporting, editorials, letters, or reaction about the supplied Major American lead candidates. Match the exact same eventKey only when it is genuinely the same event. If no same-event response can be verified, identify what an influential Black newspaper led with on the exact date; for a weekly paper, the nearest surviving issue within seven days is allowed. Give that independent story its own eventKey, comparisonType="community_lead", and dateRelation="nearest_weekly_issue" when off-date so it is never presented as a response. Set sourceDesk="black_press". For y200, use Black-authored primary records only when no Black newspaper existed; label community as voices_beyond_newsprint. If majorAmericanLeadCandidates is empty because the Major Press desk is still running, independently research the strongest nationally important same-date events and use precise eventKey values so Source Verification can reconcile same-event coverage later.',
...
FALLBACK RULE: For each community searched, when no same-event coverage can be verified, return at most ONE leading verified headline from that community press for the exact historical date. For a weekly community newspaper, the nearest surviving issue within seven days is allowed only when marked dateRelation="nearest_weekly_issue". Mark every fallback comparisonType="community_lead". Same-event candidates should use comparisonType="same_event" and dateRelation="exact_date". A fallback headline is NOT a point of view on the major American headline and must never be described that way.
...
Each candidate must use {"eraKey":"${eraKey}","eraYear":${year},"eventKey":"stable_same_event_slug","sourceDesk":"major_press","comparisonType":"unmatched","coverageScope":"national|multi_state|regional|local","nationalImportance":0,"title":"","summary":"","publication":"","city":"","issueDate":"YYYY-MM-DD or null","page":"","archive":"","sourceUrl":"","community":"","language":"","articleType":"news|editorial|letter|advertisement|reprint|other","evidenceNotes":"","confidence":0}.`,
    input: JSON.stringify({
      editionDate: context.editionDate,
      eraKey,
      year,
...
4. Identify the best verified lead story for y200 and y75, and identify which y100 national lead event has the strongest SAME-EVENT community coverage.
5. Community voices may differ sharply in framing or point of view, but must concern the same y100 event when labeled as a response.
6. A different-event community lead must retain its own eventKey and comparisonType="community_lead". The nearest surviving weekly issue within seven days is allowed only with dateRelation="nearest_weekly_issue".
7. Preserve sourceDesk, coverageScope, and nationalImportance.
...
4. Preserve eventKey unless the evidence shows it is wrong. For y100 supporting/community candidates, compare against the supplied Major Press anchors and normalize to an anchor eventKey ONLY when they truly cover the same real-world event.
5. NEVER merge merely related, same-day, same-city, or thematically similar stories.
6. Reject unsupported, invented, mismatched-date, or unverifiable candidates rather than filling gaps. The sole date exception is a verified community_lead from the nearest surviving weekly issue within seven days, explicitly marked dateRelation="nearest_weekly_issue"; it must never be normalized to a same-event response without evidence.
...
Every verifiedStories item must preserve or provide: sourceUrl, publication, issueDate, page, archive, community, language, articleType, eraKey, eraYear, eventKey, sourceDesk, coverageScope, nationalImportance, title, summary, confidence, verificationNotes.`,
...
- An independent community lead may appear in the surrounding representative press slate only when labeled comparisonType="community_lead", never as a response or point of view on the major story.
...
- If no verified same-event community coverage exists, publish the national major headline without fabricating a comparison and show clearly labeled leading stories from representative community newspapers when verified.
...
For edition.communityTiles, prefer a same-event story for each community, beginning with the community most directly implicated by the lead. If no verified same-event story exists, the tile may contain that community's leading verified headline for the exact historical date, or the nearest surviving weekly issue within seven days, and must be marked comparisonType="community_lead" with dateRelation="nearest_weekly_issue" when applicable. Never describe a fallback as a point of view on the national lead. The featured Black Press story in stories.y100.black remains same-event only; an independent Black Press lead belongs in communityTiles.
