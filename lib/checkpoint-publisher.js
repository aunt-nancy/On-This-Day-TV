...
import { exactDateEdition, exactDateStory } from './exact-date.js';
...
function normalizeEdition(raw={},verifiedUrls=new Set(),editionDate=''){
  const s=raw?.stories||{}; const y100=s?.y100||{};
  const keep=(story,eraKey)=>{const n=normalizeStory(story);return n&&verifiedUrls.has(n.sourceUrl)&&exactDateStory(n,eraKey,editionDate)?n:null;};
  const major=keep(y100.major,'y100');
  const leadEventKey=major?.eventKey||raw.leadEventKey||'';
  let black=keep(y100.black,'y100'); if(black&&leadEventKey&&black.eventKey!==leadEventKey) black=null;
  const secondary=safeArray(y100.secondary).map(story=>keep(story,'y100')).filter(Boolean);
  const communityTiles=safeArray(raw.communityTiles).map(story=>keep(story,'y100')).filter(Boolean).map(story=>({
    ...story,
    comparisonType:leadEventKey&&story.eventKey===leadEventKey?'same_event':'community_lead',
  }));
  return {
    ...raw,
    stories:{y200:keep(s.y200,'y200')||{},y100:{major:major||{},black:black||{},secondary},y75:keep(s.y75,'y75')||{}},
...
function assemble(run,rows,verifiedMeta={}){
  const exact=rows.filter(s=>s?.eraKey&&exactDateStory(s,s.eraKey,run.edition_date));
  const requested=verifiedMeta?.recommendedLeadByEra||{};
  const y200=best(exact,'y200',s=>s.sourceDesk==='major_press'&&(!requested.y200||s.eventKey===requested.y200))||best(exact,'y200',s=>s.sourceDesk==='major_press')||best(exact,'y200');
  const y75=best(exact,'y75',s=>s.sourceDesk==='major_press'&&(!requested.y75||s.eventKey===requested.y75))||best(exact,'y75',s=>s.sourceDesk==='major_press')||best(exact,'y75');
  const y100Major=best(exact,'y100',s=>s.sourceDesk==='major_press'&&(!requested.y100||s.eventKey===requested.y100))||best(exact,'y100',s=>s.sourceDesk==='major_press')||best(exact,'y100');
  const leadEventKey=y100Major?.eventKey||requested.y100||verifiedMeta?.recommendedLeadEventKey||'';
  const y100Black=best(exact,'y100',s=>s.sourceDesk==='black_press'&&(!leadEventKey||s.eventKey===leadEventKey));
  const used=new Set([y200,y75,y100Major,y100Black].filter(Boolean).map(storyId));
  const secondary=exact.filter(s=>s.eraKey==='y100'&&!used.has(storyId(s))).sort((a,b)=>score(b)-score(a)).slice(0,6);
  const communities=new Map();
  for(const story of exact.filter(s=>s.eraKey==='y100')){
  ...
  return {
    ...
    sourceSummary:exact.map(s=>({publication:s.publication,sourceUrl:s.sourceUrl,eraKey:s.eraKey,eventKey:s.eventKey})),
...
export async function publishFromAgentCheckpoint(run){
  ...
  const existingDate=exactDateEdition(existing?.payload||{},run.edition_date);
  if(existing?.status==='published'&&editionQuality(existing.payload||{}).publishable&&existingDate.publishable){
    return {...checkpoint,published:false,reason:'already_publishable',editionId:existing.id,quality:editionQuality(existing.payload||{}),exactDate:existingDate};
  }
  ...
  if(editor){
    const candidate=normalizeEdition(editor,verifiedUrls,run.edition_date);
    ...
    if(editionQuality(candidate).publishable&&exactDateEdition(candidate,run.edition_date).publishable){payload=candidate;method='closing_desk_checkpoint';}
  }
  if(!payload){
    const candidate=assemble(run,verified,verifiedMeta);
    if(editionQuality(candidate).publishable&&exactDateEdition(candidate,run.edition_date).publishable){payload=candidate;method='verified_source_checkpoint';}
  }
  if(!payload){
    const candidate=assemble(run,verified,verifiedMeta);
    const quality=editionQuality(candidate);
    const exactDate=exactDateEdition(candidate,run.edition_date);
    if(existing?.status==='published') await update('otd_editions',`id=eq.${existing.id}`,{status:'preparing',published_at:null,lead_headline:'',updated_at:new Date().toISOString()}).catch(()=>{});
    return {...checkpoint,published:false,reason:'checkpoint_missing_exact_date_core_story',quality,exactDate};
  }
  ...
  const exactDate=exactDateEdition(payload,run.edition_date);
  if(!exactDate.publishable) return {...checkpoint,published:false,reason:'exact_date_gate_failed',quality,exactDate};
  ...
  if(!edition) return {...checkpoint,published:false,reason:'checkpoint_edition_write_failed',quality,exactDate};
  ...
  return {...checkpoint,published:true,reason:'checkpoint_published',method,editionId:edition.id,quality,exactDate,storyCount:rows.length};
}
