export const AGENT_VERSION = '2026-08-30.19agent-expanded-historic-sources.1';

export function environmentStatus() {
  const required = {
    OTD_OPENAI_KEY: Boolean(process.env.OTD_OPENAI_KEY || process.env.OPENAI_API_KEY),
    SUPABASE_URL: Boolean(process.env.SUPABASE_URL),
    SUPABASE_SERVICE_ROLE_KEY: Boolean(process.env.SUPABASE_SERVICE_ROLE_KEY),
    OTD_ADMIN_TOKEN: Boolean(process.env.OTD_ADMIN_TOKEN || process.env.ADMIN_TOKEN),
    OTD_CRON_SECRET: Boolean(process.env.OTD_CRON_SECRET || process.env.CRON_SECRET),
  };
  const social = {
