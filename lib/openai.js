...
function requestTimeoutMs(override, webSearch=false) {
  const configured = Number(override || process.env.OTD_OPENAI_TIMEOUT_MS || 80000);
  // Historical archive discovery is materially slower than a normal model call.
  // The newsroom worker is allowed 300s on Vercel and runs at most two web-search
  // agents concurrently, so give any search-backed desk a real 150s window.
  // This also prevents the hard-coded 55s/60s research limits from trapping a
  // difficult date in an endless retry loop.
  if (webSearch) return Math.max(150000, Math.min(configured, 180000));
  return Math.max(20000, Math.min(configured, 110000));
}
