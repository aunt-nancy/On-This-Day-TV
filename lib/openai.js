...
function requestTimeoutMs(overrideMs) {
  const configured = Number(overrideMs ?? process.env.OTD_OPENAI_TIMEOUT_MS ?? 80000);
  return Math.max(15000, Math.min(configured, 110000));
}
...
export async function runModel({
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    ...
    const requestMs = requestTimeoutMs(timeoutMs);
    ...
    try {
      ...
      if (error?.name === 'AbortError') {
        throw new Error(`OpenAI request timed out after ${Math.round(requestMs / 1000)} seconds`);
      }
      throw error;
    } finally {
      clearTimeout(timer);
