...
export function communityDateStory(story,editionDate){
  if(story?.comparisonType==='same_topic'&&!String(story?.topicKey||'').trim())return false;
  if(exactDateStory(story,'y100',editionDate))return true;
  const issueDate=normalizeDate(story?.issueDate);
  const expected=expectedIssueDate(editionDate,'y100');
  if(
    story?.title&&story?.sourceUrl&&story.comparisonType==='same_topic'&&
    ['previous_daily_issue','adjacent_daily_issue'].includes(story.dateRelation)&&
    dayDistance(issueDate,expected)<=2
  )return true;
  if(
    story?.title&&story?.sourceUrl&&story.comparisonType==='same_topic'&&
    story.dateRelation==='nearest_weekly_issue'&&dayDistance(issueDate,expected)<=7
  )return true;
  return Boolean(
    story?.title&&story?.sourceUrl&&
    story.comparisonType==='community_lead'&&
    story.dateRelation==='nearest_weekly_issue'&&
    dayDistance(issueDate,expected)<=7
  );
}

export function communityRelationshipStory(story,lead={},editionDate){
  if(!communityDateStory(story,editionDate))return false;
  const comparisonType=String(story?.comparisonType||'');
  const leadEventKey=String(lead?.eventKey||'');
  const leadTopicKey=String(lead?.topicKey||'');
  if(comparisonType==='same_event'){
    return Boolean(leadEventKey&&story.eventKey===leadEventKey&&exactDateStory(story,'y100',editionDate));
  }
  if(comparisonType==='same_topic'){
    return Boolean(leadTopicKey&&story.topicKey===leadTopicKey);
  }
  if(comparisonType==='community_lead'){
    const searchedTopic=String(story?.searchedTopicKey||'');
    return Boolean(
      story.searchOutcome==='same_topic_not_verified'&&
      String(story.sourceSelectionNote||'').trim()&&
      (!leadTopicKey||!searchedTopic||searchedTopic===leadTopicKey)
    );
  }
  return false;
}
...
export function sanitizeExactDateEdition(payload={},editionDate=payload?.editionDate||''){
  ...
  const lead=clean.stories.y100.major||{};
  if(clean.stories.y100.black?.title&&(
    !lead.eventKey||clean.stories.y100.black.eventKey!==lead.eventKey
  )){
    invalid.push({
      label:'y100_black',
      title:clean.stories.y100.black.title,
      publication:clean.stories.y100.black.publication||'',
      issueDate:normalizeDate(clean.stories.y100.black.issueDate),
      expectedIssueDate:expectedIssueDate(editionDate,'y100'),
      sourceUrl:clean.stories.y100.black.sourceUrl,
  ...
  clean.communityTiles=(Array.isArray(clean.communityTiles)?clean.communityTiles:[])
    .map((story,i)=>{
      const normalized=!story?.comparisonType&&lead.eventKey&&story?.eventKey===lead.eventKey
        ?{...story,comparisonType:'same_event'}
        :story;
      if(communityRelationshipStory(normalized,lead,editionDate))return normalized;
      if(story?.title&&story?.sourceUrl){
        invalid.push({
          ...
          expectedIssueDate:expectedIssueDate(editionDate,'y100'),
          sourceUrl:story.sourceUrl,
          reason:'date_or_relationship_invalid',
        });
      }
      return null;
    }).filter(Boolean);
  ...
  if(core.y100){
    ...
    clean.leadTopicKey=clean.stories.y100.major.topicKey||clean.leadTopicKey||'';
  }else{
    clean.leadHeadline='';
    clean.leadEventKey='';
    clean.leadTopicKey='';
