...
function rankLockedCommunityTiles(){
  ...
  cards.slice(2).forEach(card=>{ if(anchor){ anchor.after(card); anchor = card; } });
}
...
function paperParts(paper){
  if(!paper) return {};
  return {name:paper.querySelector('.paper-name'),date:paper.querySelector('.paper-date'),headline:paper.querySelector('.paper-headline'),copy:paper.querySelector('.paper-copy'),link:paper.querySelector('.source-link')};
}
...
function applyGeneratedIllustration(selector, illustration){
  ...
  el.setAttribute('role','img'); el.setAttribute('aria-label', illustration.label || 'Editorial illustration');
  el.title = illustration.label || 'Editorial illustration'; el.dataset.generatedIllustration = 'true';
}
function renderThenNow(data){
  ...
  if(sources){sources.replaceChildren();data.sources.filter(s=>text(s?.url)).slice(0,4).forEach((s,i)=>{if(i)sources.append(document.createTextNode(' • '));const a=document.createElement('a');a.href=s.url;a.target='_blank';a.rel='noopener noreferrer';a.textContent=s.label||'Source';sources.append(a);});}
...
(async function loadPublishedEditionIntoLockedDesign(){
  try{
    ...
    applyGeneratedIllustration('.era-200 .paper-illustration',edition?.illustrations?.y200);
    applyGeneratedIllustration('.era-76 .paper-illustration',edition?.illustrations?.y75);
    const pool = [...(black ? [black] : []),...(Array.isArray(stories.y100?.secondary) ? stories.y100.secondary : []),...(Array.isArray(edition.communityTiles) ? edition.communityTiles : [])].filter(Boolean);
    const leadEventKey = text(edition.leadEventKey || major?.eventKey);
    const candidatesByCommunity = new Map();
    for(const story of pool){const key=normalizeCommunity(story.community||story.communityKey||story.label||story.group);if(!key||!text(story.title))continue;if(!candidatesByCommunity.has(key))candidatesByCommunity.set(key,[]);candidatesByCommunity.get(key).push(story);}
    function chooseCommunityStory(key){const candidates=candidatesByCommunity.get(key)||[];if(!candidates.length)return null;const sameEvent=candidates.filter(story=>leadEventKey&&text(story.eventKey)===leadEventKey).sort((a,b)=>Number(b.confidence||0)-Number(a.confidence||0));if(sameEvent.length)return {...sameEvent[0],comparisonType:'same_event'};const fallback=[...candidates].sort((a,b)=>Number(b.confidence||0)-Number(a.confidence||0))[0];return fallback?{...fallback,comparisonType:'community_lead'}:null;}
    for(const key of ['black','latino','german','british','chinese','japanese','irish','italian','jewish','indigenous','more']){
      const story=chooseCommunityStory(key);if(!story)continue;
      const card=key==='black'?document.getElementById('blackCenter'):document.querySelector(`.community-card[data-community="${key}"]`);if(!card)continue;
      card.dataset.comparisonType=story.comparisonType;
      const p=card.querySelector('p');if(p){const pub=text(story.publication),summary=text(story.summary),prefix=story.comparisonType==='same_event'?'Same-event coverage: ':'Leading community headline: ';p.textContent=`${prefix}${pub?pub+': ':''}${story.title}${summary?' — '+summary:''}`;}
      const link=card.querySelector('a');setSourceLink(link,story.sourceUrl,story.comparisonType==='same_event'?'View Original Source →':'View Leading Headline Source →');
