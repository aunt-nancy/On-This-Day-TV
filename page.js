...
async function renderToday(){
  ...
  const d=await getToday();const e=d?.edition?.payload;if(!e){mount.innerHTML='<div class="empty-state">Today’s automatic edition has not published yet.</div>';return}
  const s=e.stories||{};const y=s.y100||{};
  mount.innerHTML=`<div class="page-kicker">${esc(fmtDate(e.editionDate))}</div><h1 class="page-title">Today’s Edition</h1><p class="page-intro">One date. Three eras. Many American voices. Every displayed story links back to the source record used by the newsroom.</p><div class="editorial-grid">${storyCard(s.y200,'200 Years Ago')}${storyCard(y.major,'100 Years Ago — Major American Press')}${storyCard(y.black,'100 Years Ago — Black Press')}${storyCard(s.y75,'75 Years Ago')}</div>${(y.secondary||[]).length?`<h2>More Verified Headlines</h2><div class="editorial-grid">${(y.secondary||[]).map(x=>storyCard(x,'Additional Headline')).join('')}</div>`:''}`;
}
...
async function renderCommunity(){
  ...
  if(!items.length){mount.innerHTML='<div class="empty-state">Community Press Voices will populate automatically when verified source material is available.</div>';return}
  mount.innerHTML=`<div class="community-list">${items.map(s=>`<article class="editorial-card"><div class="meta">${esc(communityName(s))}</div><h2>${esc(s.title)}</h2><p>${esc(s.summary||'')}</p><p class="muted">${s.comparisonType==='same_event'?'Verified coverage of the featured event':'Leading verified community headline for the date'}</p>${s.sourceUrl?`<a target="_blank" rel="noopener" href="${esc(s.sourceUrl)}">View original source →</a>`:''}</article>`).join('')}</div>`;
}
async function renderArchive(){
  ...
  mount.innerHTML=rows.length?rows.map(e=>`<div class="archive-row"><b>${esc(fmtDate(e.edition_date))}</b><span>${esc(e.lead_headline||'Edition')}</span><a href="today.html?date=${esc(e.edition_date)}">Open edition →</a></div>`).join(''):'<div class="empty-state">Published editions will appear here automatically.</div>';
}
async function renderRegional(){
  ...
  mount.innerHTML=rows.length?`<div class="editorial-grid">${rows.map(s=>storyCard(s,'Regional & Local')).join('')}</div>`:'<div class="empty-state">No additional verified regional/local stories are published for the current edition.</div>';
}
